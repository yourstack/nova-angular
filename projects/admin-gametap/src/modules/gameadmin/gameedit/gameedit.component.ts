...
@Component({
  ...
  styleUrls: ['./gameedit.component.scss']
})
export class GameeditComponent {
  ...
  async save(){
    let objectId = this.game.objectId || ""
    let jsonData:any = {}
    jsonData = this.game
    // 删除系统保留字段
    ...
    // 发送新增/更新请求
      ...
      let httpMethod = "PUT"
      let url = `http://metapunk.cn:9999/parse/classes/Game/${objectId}`
      if(!this.isUpdate){
        httpMethod = "POST"
        url = `http://metapunk.cn:9999/parse/classes/Game`
      }
  ...
  async getGameById(id:string){
    let response = await fetch(`http://metapunk.cn:9999/parse/classes/Game/${id}?`, {
      "headers": {
        "accept": "*/*",
